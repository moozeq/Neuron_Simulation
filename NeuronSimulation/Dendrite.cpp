...
void Dendrite::addDisc(std::vector<UCoord3>& indices, unsigned midPointIndex)
{
	...
	length(_length), slices(30)
{
	...
	startCoords[0] = x0 - _length / 2.0f - _lipidBilayerWidth / 2.0f;
	startCoords[1] = y0;
	startCoords[2] = z0;

	stopCoords[0] = x0 + _length / 2.0f + _lipidBilayerWidth / 2.0f;
	stopCoords[1] = y0;
	stopCoords[2] = z0;
...
collision::Type Dendrite::checkCollision(const float newCoords[3], const float oldCoords[3]) const
{
	...
	if (oldD < innerRadius &&
		newD < innerRadius &&
		newCoords[0] < startCoords[0] + lipidBilayerWidth &&
		oldCoords[0] > startCoords[0] + lipidBilayerWidth)
...
int Dendrite::getCollisionPoint(float* point, float newCoords[3], float oldCoords[3], collision::Type collisionType) const
{
...
bool Dendrite::getRandPointOnInnerLayer(float* point, glm::vec3& inOutVec) const
{
	...
	if (synapse < synapseProbability) {
		x = startCoords[0] + lipidBilayerWidth;
...
void Dendrite::setSynapseProbability(float probability)
{
	synapseProbability = probability;
}

float* Dendrite::getSynapsePoint()
{
	return startCoords;
}
