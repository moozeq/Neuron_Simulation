...
GLuint Nucleus::generateLayer(std::vector<FCoord8>& vertices, std::vector<UCoord3>& indices)
{
...
Nucleus::Nucleus(float coords[3], float _radius, float _lipidBilayerWidth)
{
	precision = 0.0001f;
	radius = _radius;
	lipidBilayerWidth = _lipidBilayerWidth;
	innerRadius = outerRadius = radius = _radius;
	x0 = coords[0];
	y0 = coords[1];
	z0 = coords[2];
...
collision::Type Nucleus::checkCollision(const float newCoords[3], const float oldCoords[3]) const
{
	...
	if (newD < radius && oldD > radius)
		return collision::OUTSIDE;

	return collision::NONE;
}

int Nucleus::getCollisionPoint(float* point, float newCoords[3], float oldCoords[3], collision::Type collisionType) const
{
...
bool Nucleus::getCollisionNormalVec(float collisionPoint[3], glm::vec3& n, collision::Type collisionType) const
{
	glm::vec3 midPoint = glm::vec3(x0, y0, z0);
	n = glm::normalize(midPoint - glm::vec3(collisionPoint[0], collisionPoint[1], collisionPoint[2]));

	return true;
}

bool Nucleus::getRandPointOnInnerLayer(float* point, glm::vec3& inOutVec) const
{
